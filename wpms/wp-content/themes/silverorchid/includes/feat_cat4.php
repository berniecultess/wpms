 ...
 **/
	...
	$cat4_id = $gazpo_settings['gazpo_feat_cat4'];
	$cat4_name = get_cat_name($cat4_id);
	$cat4_url = get_category_link( $cat4_id );
?>

<div class="feat-cat">
	<h3 class="title"><a href="<?php echo esc_url( $cat4_url ); ?>" title="Category Name"><?php echo $cat4_name; ?></a></h3>
		<div class="category-content whitebg">
			<div class="left">	
				<?php 
					query_posts('cat='.$cat4_id.'&posts_per_page=1');
					if (have_posts()) :
			...
			<div class="right">
				<ul>
				<?php 
					query_posts('cat='.$cat4_id.'&posts_per_page=3&offset=1');
					if (have_posts()) :
						while (have_posts()) : the_post(); 
				?>
					<li>
