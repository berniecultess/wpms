 ...
 **/
	...
	$cat3_id = $gazpo_settings['gazpo_feat_cat3'];
	$cat3_name = get_cat_name($cat3_id);
	$cat3_url = get_category_link( $cat3_id );
?>

<div class="feat-cat">
	<h3 class="title"><a href="<?php echo esc_url( $cat3_url ); ?>" title="Category Name"><?php echo $cat3_name; ?></a></h3>
		<div class="category-content whitebg">
			<div class="left">	
				<?php 
					query_posts('cat='.$cat3_id.'&posts_per_page=1');
					if (have_posts()) :
			...
			<div class="right">
				<ul>
				<?php 
					query_posts('cat='.$cat3_id.'&posts_per_page=3&offset=1');
					if (have_posts()) :
						while (have_posts()) : the_post(); 
				?>
					<li>
