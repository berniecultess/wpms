...
<div id="content"><?php
	...
	<div class="postFrame">
		...
		<div class="postContent">
			...
			echo $theTitle = the_title('<h2 class="postTitle">'.(is_single() || is_page()? '' : '<a href="'.get_permalink().'">'),(is_single() || is_page()? '' : '</a>').($page >= 2 ? __(' » Page').' '.$page: '').'</h2>',false);
			if (strtolower($post->post_type) == "post" && $theTitle) { ?>
			<div class="postMeta">
				<?php _e('By');?> <a href="<?php echo get_author_posts_url(get_the_author_ID()); ?>"><?php the_author();?></a>
			...
			<div class="postFooter">
				...
				if (function_exists('the_tags'))
					the_tags('<div class="postTags"><strong>'.__('tags:').'</strong> ',', ','</div>');?>
	...
	<div class="postFrame">
		<span class="postFrameTop"><span class="IEFix"></span></span>
			<div class="postContent">
				<h2><?php _e('Sorry');?>:</h2>
				<h3><?php _e("We couldn't find what you were looking for.");?></h3>
	...
	if (function_exists('paginate_archives'))
		...
		if ( $wp_query->max_num_pages > 1 && !is_singular()) {?>
			<div id="pageNavigation">
				<div id="previousPosts"><?php previous_posts_link(__('&laquo; Previous page'));?></div>
				<div id="nextPosts"><?php next_posts_link(__('Next page &raquo;'));?></div>
			</div><?php
		}
	}?>
</div>
